    ...
    public ResponseEntity<R<String>> login(@Valid @RequestBody LoginRequestDTO request) {
        ...
        try {
            Authentication authentication = authenticationManager.authenticate(
                    new UsernamePasswordAuthenticationToken(realUsername, request.getPassword()));
            loginAttemptService.loginSucceeded(realUsername);
            // 如果认证成功，生成JWT
            String token = jwtUtil.generateToken(authentication);
            return ResponseEntity.ok(R.success(token));
        } catch (BadCredentialsException e) {
            // 3. 如果认证失败（密码错误），记录失败次数
            loginAttemptService.loginFailed(realUsername);
            // 返回标准的认证失败信息

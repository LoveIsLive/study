...
public final class ExplorationPrompts {
    ...
    private static final String PROBLEM_BUNDLE_AUTHORITY_RULES =
            "ProblemBundle authority rules:\n"
                    ...
                    + "- `diagram.coordinate_model` may provide coordinates, equations, ranges, and dependency formulas extracted from the statement/source figure. When present, treat those explicit mathematical facts as hard constraints, not optional hints.\n"
                    + "- Preserve every source-resolved coordinate-model branch choice. Do not replace it with a visually plausible alternate branch when deriving steps, loci, endpoints, extrema, or final answers.\n"
                    + "- Before stating a transformed center, transformed endpoint, locus equation, extremum point, or final value, substitute the relevant fixed points and motion endpoints into the dependency formula from `diagram.coordinate_model`; keep transformed centers distinct from transformed endpoints.\n"
                    + "- If a coordinate-model formula conflicts with an intuitive mental picture or a shorter statement, follow the coordinate model and ambiguity selection.\n"
    ...
    private static final String CONCEPT_GRAPH_SYSTEM =
            "Plan a compact motion-driven teaching DAG for a middle-school math concept.\n"
                    ...
                    + "Start by building a concrete visual situation where the concept naturally arises. When useful, follow with a separate motion or manipulation beat to reveal the relationship, invariant, or pattern the concept captures. Preserve this separation between setup and motion-revealed meaning when it improves clarity.\n"
                    + "Keep setup, motion reveal, and formal naming separate when that makes the concept easier to see.\n"
                    + "Prioritize movable visual elements over text when motion or manipulation clarifies the concept.\n"
    ...
    private static final String PROBLEM_GRAPH_SYSTEM =
            "Plan a compact motion-driven solving DAG for a middle-school math problem.\n"
                    ...
                    + "Start by building the problem situation as a concrete visual setup, then reveal what quantity, relation, or target the problem is asking about.\n"
                    + "Prioritize movable problem quantities and visual elements over text when motion clarifies the target or key insight.\n"
                    ...
                    + "Node rules:\n"
                    + "1. Each node must be one atomic solving beat with one main takeaway.\n"
                    ...
                    + "3. Setting up the problem situation and revealing the question's target through motion are separate atomic beats when useful; this helps the learner first understand the problem, then understand what is being optimized, proven, or found.\n"
                    ...
                    + "Coordinate-model discipline:\n"
                    + "1. If `diagram.coordinate_model` contains explicit coordinates, equations, ranges, or dependency formulas, derive the solution path from those formulas before writing presentation steps.\n"
                    + "2. For transformation, locus, or optimization problems, perform compact substitution checks for fixed centers and motion endpoints before naming derived coordinates, equations, valid arcs, or extrema.\n"
                    + "3. If a step uses a transformed object, state only facts consistent with the dependency formula; do not mix the image of a center with the image of an endpoint.\n"
                    + "4. If a branch is source-resolved in `diagram.ambiguities`, keep that branch through the whole graph, including all derived coordinates and final values.\n\n"
                    + PROBLEM_BUNDLE_AUTHORITY_RULES
                    + "Edge rules:\n"

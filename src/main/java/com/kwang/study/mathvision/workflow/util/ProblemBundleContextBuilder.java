...
public final class ProblemBundleContextBuilder {
    ...
    public static boolean isProblemMode(ProblemBundle bundle) {
        ...
        String inputMode = normalizeInputMode(bundle.getInputMode());
        if ("problem".equals(inputMode)) {
            return true;
        }
        if ("concept".equals(inputMode)) {
            return false;
        }

        String normalized = displayTitle(bundle).trim().toLowerCase(Locale.ROOT);
    ...
    public static String normalizeInputMode(String mode) {
        String normalized = mode == null ? "" : mode.trim().toLowerCase(Locale.ROOT);
        if ("concept".equals(normalized) || "problem".equals(normalized)) {
            return normalized;
        }
        return "auto";
    }

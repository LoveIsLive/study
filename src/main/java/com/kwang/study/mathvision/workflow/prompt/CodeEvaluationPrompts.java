...
public final class CodeEvaluationPrompts {
    ...
    private static final String REVIEW_SYSTEM_MANIM =
            "You are a senior Manim code reviewer.\n"
                    ...
                    + "- `manim_coordinate_mapping` [MANDATORY]: when storyboard `coordinate_bounds` exists, storyboard world-coordinate geometry must be mapped through a coordinate system with one uniform x/y unit scale so equal world lengths render equally and the padded bounds fit inside the Manim frame. Points must use `axes.c2p(...)` or a clearly named helper wrapping `c2p`; raw Manim frame coordinates are allowed only for fixed overlays, titles, camera/UI placement, and other non-storyboard-geometry elements. Fail this check when points, lines, curves, polygons, markers, or geometry-attached labels use raw coordinates such as `Dot([x, y, 0])`, `Line([x1, y1, 0], [x2, y2, 0])`, or `.move_to([x, y, 0])` instead of the coordinate-system mapping. Also fail this check when the code uses non-uniform `x_length`/`y_length` for storyboard geometry, or when storyboard world radii/metric lengths are passed directly to `Circle(radius=...)`, `Arc(radius=...)`, or similar geometry without converting through the same uniform unit scale.\n"
    ...
    private static final String REVIEW_SYSTEM_GEOGEBRA =
            "You are a senior GeoGebra construction reviewer.\n"
                    ...
                    + "- For `moves_on_object` or `lies_on` constraints: the object must be constructed with documented dependency-preserving GeoGebra syntax such as `Point(path)` or `PointIn(region)`, not undocumented `PointOn` syntax and not free coordinates. For direct movement, dependency-preserving is not enough: the point must also remain directly draggable, so `Point(path, numericConstant)` is a failure for movable/draggable points.\n"
                    + "- `geogebra_interactivity` [MANDATORY]: GeoGebra is an interactive construction medium. For every storyboard object with `moves_on_object`, `moves_along_range`, `slider_driven`, `follows_path`, or explicit movable/draggable semantics, verify that the command script preserves the intended interaction affordance.\n"
                    + "- Direct draggable constrained points must be implemented with documented direct path-point syntax such as `Point(path)` or `PointIn(region)`.\n"
                    + "- Fail `geogebra_interactivity` if a directly movable/draggable storyboard point is defined as `Point(path, numericConstant)`, `Point({x, y})`, `Intersect(...)`, `Reflect(...)`, `Midpoint(...)`, or any other static/dependent construction that removes direct dragging.\n"
                    + "- Fail `geogebra_interactivity` if `SetFixed(object, true)` is applied to an object that the storyboard says should be movable/draggable.\n"
                    + "- Treat `Point(path, slider)` as slider-driven rather than direct point dragging; accept it only when the storyboard explicitly intends slider control and the slider is visible/usable.\n"
                    ...
                    + "- `viewport_contract` [MANDATORY]: the initial visible coordinate window must come from storyboard `coordinate_bounds`; the command script must contain a `SetCoordSystem(x_min, x_max, y_min, y_max)` whose values match storyboard bounds, important objects should fit strictly inside this view without relying on user zooming or panning, and the construction should not be tiny or clustered inside an over-wide view.\n"
                    ...
                    + "- GeoGebra is interactive, and required interactivity is part of the semantic contract. Also check initial-view readability and construction coherence; do not approve a readable static construction when the storyboard requires movable/draggable exploration.\n\n"
                    + REVIEW_API_WHITELIST_WARNING_POLICY
                    + REVIEW_OUTPUT_SCHEMA;
    ...
    public static String revisionUserPrompt(String sceneName,
        ...
        if ("geogebra".equalsIgnoreCase(outputTarget)) {
            return SystemPrompts.buildCurrentRequestSection(String.format(
                    "Figure name: %s\n\n"
                            ...
                            + "If a storyboard point is movable/draggable or has motion constraints, preserve the intended interaction: use `Point(path)` for direct draggable constrained points, and do not replace them with `Point(path, numericConstant)` or static dependent constructions unless the review explicitly says the storyboard intended a fixed point.\n"
                            ...
                            + "Preserve the storyboard `coordinate_bounds` viewport contract with `SetCoordSystem(x_min, x_max, y_min, y_max)`, and fix layout by scaling/spreading/recentering the construction or updating the bounds coherently rather than relying on user zoom.\n"
        ...
        return SystemPrompts.buildCurrentRequestSection(String.format(
                "Scene class name: %s\n\n"
                        ...
                        + "Use object_registry constraints, scene placement/style, and scene notes_for_codegen as semantic guidance; treat placement as preferred layout input that may be adjusted for safety/readability, and never force a derived object to a placement coordinate when structured constraints or notes_for_codegen define a different construction.\n"
                        + "When storyboard coordinate_bounds are present in Manim code, preserve a single uniform x/y unit scale for storyboard geometry, and convert storyboard world radii/metric lengths through the same scale before passing them to Circle, Arc, sectors, braces, or similar geometry.\n"

...
public final class CodeGenerationPrompts {
    ...
    private static final String MANIM_CODE_GENERATION_SYSTEM =
            "You are an expert Manim Community engineer and Python programmer.\n"
                    ...
                    + "- When `coordinate_bounds` exists, the generated Manim code MUST define or use a shared coordinate system object (`Axes`, `NumberPlane`, or in 3D `ThreeDAxes`/equivalent) from those bounds with one uniform x/y unit scale, so equal storyboard world lengths render equally and the padded coordinate bounds fit inside the Manim frame.\n"
                    + "- Map storyboard world points through `axes.c2p(...)` or a clearly named helper that wraps `c2p`; for storyboard world radii or metric lengths used by `Circle`, `Arc`, sectors, braces, or similar geometry, convert the value through the same uniform unit scale instead of passing it as a raw Manim frame radius.\n"
                    ...
                    + "- When implementing at least one selected important `voiceover_text`, use `VoiceoverScene`, import `VoiceoverScene` from `manim_voiceover` and `EdgeTTSService` from `mathvision_edge_tts`, define `VOICEOVER_SPEED = 1.5`, call `self.set_speech_service(EdgeTTSService(voice=\"zh-CN-XiaoxiaoNeural\", global_speed=VOICEOVER_SPEED))`, and synchronize selected important narrated beats with `with self.voiceover(text=...) as tracker:`. Never use `GTTSService` or import `manim_voiceover.services.gtts`.\n"
    ...
    public static String manimValidationFixUserPrompt(String sceneName,
        ...
        return SystemPrompts.buildCurrentRequestSection(String.format(
                "The generated Manim code failed validation checks.\n\n"
                        ...
                        + "Preserve Chinese visible object content. Preserve only the selected important `voiceover_text` items that remain implemented; when voiceover is present, keep the `VoiceoverScene`/`EdgeTTSService`/`self.voiceover(...)` structure synchronized with those selected action beats and migrate any legacy `GTTSService` usage to `EdgeTTSService`.\n"
    ...
    public static String manimSceneCodeUserPrompt(String sceneJson,
        ...
        return SystemPrompts.buildCurrentRequestSection(String.format(
                "Now implement scene method `%s` (scene %d of %d).\n\n"
                        ...
                        + "- The static skeleton already owns imports, MainScene, construct(), `self.objects`, and shared helpers. Use `self.world_point(...)` / `self.c2p(...)` for storyboard world points when coordinate bounds are present.\n"
                        + "- The static skeleton maps storyboard coordinates with one uniform unit scale. Use `self.world_radius(r)` or the convenience helpers `self.world_circle(x, y, r, **kwargs)` and `self.world_arc(x, y, r, ...)` for storyboard world radii/metric lengths; do not pass storyboard radius values directly as raw Manim `Circle(radius=...)` or `Arc(radius=...)` frame units.\n"
                        + "- Store newly created storyboard objects with `self.register_object(\"id\", mobject)` or `self.objects[\"id\"] = mobject`; retrieve existing storyboard objects with `self.get_object(\"id\")` or `self.objects[\"id\"]`.\n"
                        + "- Do not define imports, classes, construct(), axes setup, coordinate helper methods, or global constants in the scene method body.\n"

...
public final class ManimCodeUtils {
    ...
    private static final String PYTHON_SYNTAX_PREFIX =
            "Static rule violation: invalid Python syntax";
    ...
    private static final Pattern ANIMATE_METHOD_ASSIGNMENT = Pattern.compile(
            "\\.animate\\.[A-Za-z_][A-Za-z0-9_]*\\s*=");
    ...
    public static List<String> validateManimRules(String manimCode) {
        ...
        for (String evidence : findAllAnimateMethodAssignments(manimCode)) {
            violations.add(PYTHON_SYNTAX_PREFIX
    ...
    private static List<String> findAllAnimateMethodAssignments(String manimCode) {
        List<String> evidences = new ArrayList<>();
        if (manimCode == null || manimCode.isBlank()) {
            return evidences;
        }
        ...
        for (int i = 0; i < lines.length; i++) {
            String rawLine = lines[i] != null ? lines[i] : "";
            String line = maskPythonStringsAndComments(rawLine);
            if (ANIMATE_METHOD_ASSIGNMENT.matcher(line).find()) {
                evidences.add("line " + (i + 1) + ": " + summarizeSnippet(rawLine));
            }
        }
        return evidences;
    }

    private static List<String> findAllRawStringTrailingBackslashes(String manimCode) {
        List<String> evidences = new ArrayList<>();
        if (manimCode == null || manimCode.isBlank()) {
            return evidences;
        }
        ...
        for (int i = 0; i < lines.length; i++) {
            String rawLine = lines[i] != null ? lines[i] : "";
            if (hasRawStringTrailingBackslash(rawLine)) {
                evidences.add("line " + (i + 1) + ": " + summarizeSnippet(rawLine));
            }
        }
        return evidences;
    ...
    private static boolean hasRawStringTrailingBackslash(String rawLine) {
        if (rawLine == null) {
            return false;
        }
        ...
        int rawQuoteIndex = findRawStringOpeningQuote(trimmed);
        if (rawQuoteIndex < 0) {
            return false;
        }
        char quote = trimmed.charAt(rawQuoteIndex);
        if (trimmed.charAt(trimmed.length() - 1) != quote) {
            return false;
        }
        int backslashes = 0;
        for (int i = trimmed.length() - 2; i >= 0 && trimmed.charAt(i) == '\\'; i--) {
            backslashes++;
        }
        return backslashes % 2 == 1;
    }

    private static int findRawStringOpeningQuote(String line) {
        for (int i = 0; i < line.length() - 1; i++) {
            char ch = line.charAt(i);
            if (ch != 'r' && ch != 'R') {
                continue;
            }
            boolean identifierPrefix = i > 0 && isIdentifierPart(line.charAt(i - 1));
            if (identifierPrefix) {
                continue;
            }
            int quoteIndex = i + 1;
            if (quoteIndex < line.length() && isQuote(line.charAt(quoteIndex))) {
                return quoteIndex;
            }
            if (quoteIndex + 1 < line.length()
                    && isRawBytePrefix(line.charAt(quoteIndex))
                    && isQuote(line.charAt(quoteIndex + 1))) {
                return quoteIndex + 1;
            }
        }
        return -1;
    ...
    private static boolean isRawBytePrefix(char ch) {
        return ch == 'b' || ch == 'B';
    }
    ...
    /**
     * Formats static validation findings for workflow diagnostics.
     */
    public static String summarizeIssues(List<String> issues) {
        if (issues == null || issues.isEmpty()) {
            return "No static audit issues.";
        }
